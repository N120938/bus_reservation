...
int main(){
	...
	do{
		start=readbus(start);
		printf("1.Take look of all the buses and its timmings and money to be paid\n");
		...
		n=2;
			switch(n){																	//operations have done by using switch case
				...
				case 2:	
					...
					scanf("%s",source);
					printf("Enter your destination\n");									//scan source,desetination stations by user
					scanf("%s",destination);
					int i=1;
					while(i){
...
void seatsallocation(int busno,char filename2[20]){
	...
	while(ptr!=NULL){
		if(ptr->bus_no==busno){
			...
			printf("your reservation is conformed\n");
			username=usernode;
			while(usernode!=NULL){
				username=usernode;
